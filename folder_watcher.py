...
def download_yandex_file_bytes(fpath: str, direct_download_url: str = None) -> bytes:
    ...
    if direct_download_url:
        try:
            ...
            logging.warning(f"Прямое скачивание не удалось, запрашиваем по API: {e}")
    ...
    try:
        ...
        logging.error(f"[YANDEX DOWNLOAD ERROR] Ошибка скачивания '{fpath}': {e}")
    return b""
...
def upload_json_to_yandex_disk(disk_path: str, payload: dict) -> bool:
    ...
    if not YANDEX_DISK_TOKEN:
        logging.error("[YANDEX DISK UPLOAD ERROR] YANDEX_DISK_TOKEN не задан.")
        return False
    ...
    try:
        ...
        if res.status_code != 200:
            logging.error(f"[YANDEX DISK UPLOAD ERROR] Не удалось получить href ({res.status_code}): {res.text}")
            return False

        upload_href = res.json().get("href")
        if not upload_href:
            logging.error("[YANDEX DISK UPLOAD ERROR] Поле href отсутствует в ответе API.")
            return False
        ...
        if put_res.status_code in (200, 201):
            ...
            logging.error(f"[YANDEX DISK UPLOAD ERROR] PUT вернул статус {put_res.status_code}: {put_res.text}")
            return False
    except Exception as e:
        logging.error(f"[YANDEX DISK UPLOAD EXCEPTION] Исключение при загрузке '{disk_path}': {e}")
        return False
...
def build_and_upload_folder_cache(folder_path: str, folder_name: str) -> str:
    ...
    print(f"\n[ETL PIPELINE START] Обработка документов папки '{folder_name}' ({folder_path})...")
    ...
    for item in items:
        try:
            ...
            if not folder_exists(item_path):
                ...
                # Пробуем быстро сгенерировать кэш для старых папок без кэша
                try:
                    info_url = "https://cloud-api.yandex.net/v1/disk/resources"
                    headers = {"Authorization": f"OAuth {YANDEX_DISK_TOKEN}", "Accept": "application/json"}
