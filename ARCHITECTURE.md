...
9. **Landing Page Architecture, Multimedia CMS & Admin Operations Panel:**
   - **Порядок секций главной страницы (`templates/index.html`):**
     ...
     11. Подвал (`footer`)
   - **Подсистема мультимедиа контента (Media CMS & Cloud Storage):**
     - Таблицы `public_posts` и `public_library` расширены полями `cover_image_url` (VARCHAR 500), `video_url` (VARCHAR 500), `attachments` (TEXT/JSON).
     - Модуль `upload_media_file_to_yandex_disk` в `folder_watcher.py` обеспечивает защищенную загрузку медиафайлов на Яндекс.Диск в папку `disk:/uploads/` с автоматической публикацией ресурса.
     - Эндпоинт `POST /api/v1/admin/upload` (JWT `ADMIN`) сохраняет файл локально в `static/uploads/` и параллельно синхронизирует с Яндекс.Диском, возвращая готовый публичный URL.
     - В модальном окне чтения статьи (`#article-modal`) встроен медиаплеер (HTML5 Video для прямых видеофайлов / кнопки перехода к внешним платформам) и вывод обложек.
