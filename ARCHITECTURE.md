...
### 6. Релиз v7.3: Рефакторинг жизненного цикла маскота «Алик» (Hero Cleanup & Sound Showcase Docking)
1. **Жизненный цикл и FLIP-докинг маскота «Алик» (`static/js/app.js`, `templates/index.html`, `static/css/style.css`):**
   - Полное удаление статичного дубля персонажа (`#hero-alik-wrapper`, `<picture>`, `<img>`) из Hero-блока (`#hero`), центрирование первого экрана.
   - Мгновенная инициализация плавающего виджета-компаньона (`#floating-alik-widget`) на первом экране сразу при загрузке страницы с приветственной репликой диалогового облачка (`#alik-speech-bubble`).
   - Добавлен круговой плейсхолдер докинга (`#alik-dock-placeholder` / `.alik-dock-slot`) в интерактивную витрину звуков (`#characters`).
   - Двунаправленная FLIP-анимация (First, Last, Invert, Play) на аппаратных GPU-свойствах `transform` и `opacity` с расчетом дельт `getBoundingClientRect()`:
     * При скролле до секции `#characters` виджет плавно стыкуется в круговой слот витрины (`dockToSoundShowcase`), а плавающий виджет скрывается.
     * При уходе секции `#characters` из вьюпорта вверх или вниз компаньон плавно высвобождается (`undockToFloating`) и возвращается в фиксированное состояние.
   - Полная изоляция от переключателей звуков (А, О, У, Ы, Э, И) и непрерывная смена контекстных комментариев (`data-alik-comment`) при прокрутке секций лендинга.
...
2. **Автоматический фоновый автобэкап и мониторинг свежести (`alert_service.py`):**
   - Автоматический триггер `trigger_daily_backup_if_needed(interval_hours=24)`: фоновый воркер `alert_worker_loop` каждые 24 часа создает новый снимок БД без ручного вмешательства.
   - 7-я метрика мониторинга `check_backup_freshness(max_age_hours=26)`: контроль возраста последнего дампа и отправка email-алерта на `PRIMARY_ALERT_EMAIL` и `SECONDARY_ALERT_EMAIL` при отсутствии снимка > 26 часов.
3. **Административный REST API (`main.py`):**
   ...
   - `GET /api/v1/admin/alerts/status` — возвращает статусы всех 7 метрик, включая `"backup_freshness"`.
4. **Веб-интерфейс в CMS Администратора (`templates/index.html`, `static/js/app.js`):**
   ...
   - В карточке системы оповещений отображается живой индикатор свежести дампа: `🟢 Актуален (X ч)`.
   - Клиентские методы `loadAdminBackups()` и `handleCreateBackup()`.
5. **Тестовый сьют:** 118 тестов со 100% прохождением (118/118 PASS).
